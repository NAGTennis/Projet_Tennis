...
shinyServer(function(input, output, session) {
  ...
  output$Erreur <- renderText({
    paste("Erreur", input$Choix, ':', ERR2[1,input$Choix])
  ...
  output$chart_surface <- renderAmCharts({
    tab=Tennis_table[,.(Nb_Match=.N),by=surface]
    colnames(tab)=c("label","value")
    amPie(data = tab, depth=15, creditsPosition = "bottom-right")
  })
  
  output$chart_pays <- renderAmCharts({
    tab=head(rbind(merge(Tennis_table_work,atp_players,by.x="w_id", by.y="Player_Id", all.x=TRUE),merge(Tennis_table_work,atp_players,by.x="l_id", by.y="Player_Id", all.x=TRUE))[,.(nb=.N),by=Nationalite][order(-nb)],10)
    colnames(tab)=c("label","value")
    amPie(data = tab, depth=15, main='Nationalité des joueurs depuis 1991', creditsPosition = "bottom-right")
  })
  
  output$chart_vict_pays <- renderAmCharts({
    tab=merge(head(merge(Tennis_table_work,atp_players,by.x="w_id", by.y="Player_Id", all.x=TRUE)[,.(nb=.N),by=Nationalite][order(-nb)],10),merge(Tennis_table_work,atp_players,by.x="l_id", by.y="Player_Id", all.x=TRUE)[,.(nb=.N),by=Nationalite],by.x="Nationalite",by.y="Nationalite", all.x=TRUE)[order(-nb.x),.(Nationalite,tx_vict=round(nb.x/(nb.x+nb.y),2))]
    colnames(tab)=c("Pays","Tx de victoire")
  ...
  output$chart_fatigue <- renderAmCharts({
    ...
    # colnames(tab)=c("label","value")
    # amPie(data = tab, depth=15, main='Taux de victoire par rapport à la fatigue', creditsPosition = "bottom-right")
    amSolidGauge(x = round((nrow(table_score[fatigue>0&coin==0])+nrow(table_score[fatigue<0&coin==1]))/nrow(table_score[fatigue!=0])*100,2), text = "%", type = "semi", main='Taux de victoire des joueurs moins fatigués que leur adversaire')
  })
