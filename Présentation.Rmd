...
```
```{r Fréquence par pays}
#freq_pays <- data.frame(table(Tennis_hist$winner_ioc))
#new <- data.table(freq_pays[with(freq_pays,order(-Freq)),])
#new <- new[1:10,]
#p<-ggplot(data=new, aes(x=Var1,y=Freq))+geom_bar(stat="identity",fill="steelblue")+geom#_text(aes(label=Freq),vjust=-0.3,size=3.5)+theme_minimal()
#p
#p + coord_flip()
```
