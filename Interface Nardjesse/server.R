...
shinyServer(function(input, output, session) {
  ...
  output$Surface <- renderPlot({
    ...
    ggplot(table_score, aes(x="", fill=surface))+geom_bar(width = 1 )+coord_polar("y")+scale_fill_brewer(palette="Green")+
      labs(title = "Surface de jeu")+labs(x="Titre")
    
  })
  
  output$tableau <- renderTable({
    
    table(table_score$surface)
    
  })
  
  output$pays <- renderPlot({
    
    ggplot(data=new, aes(x=Var1,y=Freq))+geom_bar(stat="identity",fill="steelblue")+geom_text(aes(label=Freq),vjust=-0.3,size=3.5)
