...
```{r}
table_score=Tennis_table_work[,.(tourney_id,tourney_name,surface,tourney_date,tourney_level,match_num,round,round_num,minutes,score,best_of,w_id,w_name,w_age,w_ht,w_rank,w_rank_points,w_hand,w_seed,l_id,l_name,l_age,l_ht,l_rank,l_rank_points,l_hand,l_seed)]
...
f_fatigue(Tennis_table_work,i_name = "Rafael Nadal",i_date = '2018-05-28',i_round = 4,i_nbmatchs = 1, i_matchnum=1)
start_time <- Sys.time()
...
start_time <- Sys.time()
table_score[,c("hist_ace","hist_df","hist_1stIn","hist_1stWon","hist_2ndWon","hist_bpSaved","hist_bpMean","hist_bpConverted","hist_svptWon","hist_returnPtWon","hist_1stReturnWon","hist_2ndReturnWon","fatigue","abandon_diff"):=c((f_historique(Tennis_table_work,i_name=p1_name,i_round=round_num,i_matchnum=match_num)-f_historique(Tennis_table_work,i_name=p2_name,i_round=round_num,i_matchnum=match_num)),(f_fatigue(Tennis_table_work,i_name=p1_name,i_date=tourney_date,i_duree=NULL, i_nbmatchs=2, i_round=round, i_matchnum=match_num)[,NbJeuJoue]-f_fatigue(Tennis_table_work,i_name=p2_name,i_date=tourney_date,i_duree=NULL, i_nbmatchs=2, i_round=round, i_matchnum=match_num)[,NbJeuJoue]),(f_retour_abandon(Tennis_table_work,i_name=p1_name,i_date=tourney_date)-f_retour_abandon(Tennis_table_work,i_name=p2_name,i_date=tourney_date))),by=c(colnames(table_score))]
end_time <- Sys.time()
time=end_time - start_time

####VERIFICATION
test=table_score[tourney_id=="2000-339"&match_num==10]
test[,c("hist_ace","hist_df","hist_1stIn","hist_1stWon","hist_2ndWon","hist_bpSaved","hist_bpMean","hist_bpConverted","hist_svptWon","hist_returnPtWon","hist_1stReturnWon","hist_2ndReturnWon","fatigue","abandon_diff"):=c((f_historique(Tennis_table_work,i_name=p1_name,i_round=round_num,i_matchnum=match_num)-f_historique(Tennis_table_work,i_name=p2_name,i_round=round_num,i_matchnum=match_num)),(f_fatigue(Tennis_table_work,i_name=p1_name,i_date=tourney_date,i_duree=NULL, i_nbmatchs=2, i_round=round, i_matchnum=match_num)[,NbJeuJoue]-f_fatigue(Tennis_table_work,i_name=p2_name,i_date=tourney_date,i_duree=NULL, i_nbmatchs=2, i_round=round, i_matchnum=match_num)[,NbJeuJoue]),(f_retour_abandon(Tennis_table_work,i_name=p1_name,i_date=tourney_date)-f_retour_abandon(Tennis_table_work,i_name=p2_name,i_date=tourney_date))),by=c(colnames(test))]

test[,c("patate1"):=(f_retour_abandon(Tennis_table_work,i_name=p2_name,i_date=tourney_date)),by=c(colnames(test))]
test[,c("hist_ace1","hist_df1","hist_1stIn1","hist_1stWon1","hist_2ndWon1","hist_bpSaved1","hist_bpMean1","hist_bpConverted1","hist_svptWon1","hist_returnPtWon1","hist_1stReturnWon1","hist_2ndReturnWon1"):=c(f_historique(Tennis_table_work,i_name=p1_name,i_round=round_num,i_matchnum=match_num)),by=c(colnames(test))]
test[,c("hist_ace2","hist_df2","hist_1stIn2","hist_1stWon2","hist_2ndWon2","hist_bpSaved2","hist_bpMean2","hist_bpConverted2","hist_svptWon2","hist_returnPtWon2","hist_1stReturnWon2","hist_2ndReturnWon2"):=c(f_historique(Tennis_table_work,i_name=p2_name,i_round=round_num,i_matchnum=match_num)),by=c(colnames(test))]
```