...
#Sortir le calcul du nombre de jeux pour alléger le calcul
f_fatigue <- function(Tennis_table_work,i_name,i_date,i_duree) {
  
  historique=rbind(Tennis_table_work[winner_name==i_name,.(tourney_date,scorenum=sum(as.numeric(unlist(str_extract_all(gsub("\\(\\d\\)","",score), "[0-9]+")))),minutes),by=c(colnames(Tennis_table_work))],
  Tennis_table_work[loser_name==i_name,.(tourney_date,scorenum=sum(as.numeric(unlist(str_extract_all(gsub("\\(\\d\\)","",score), "[0-9]+")))),minutes),by=c(colnames(Tennis_table_work))])[anydate(i_date)>anydate(tourney_date)][difftime(anydate(i_date),anydate(tourney_date))<=i_duree]
  
  Historique_Sum=historique[,.(scorenum,minutes)][,lapply(.SD, function(x) sum(na.omit(x[is.finite(x)])))]
...
historique=rbind(Tennis_table[winner_name==i_name,.(tourney_date,banana=difftime(anydate(i_date),anydate(tourney_date)),scorenum=sum(as.numeric(unlist(str_extract_all(gsub("\\(\\d\\)","",score), "[0-9]+")))),minutes),by=c(colnames(Tennis_table))],
  Tennis_table[loser_name==i_name,.(tourney_date,banana=difftime(anydate(i_date),anydate(tourney_date)),scorenum=sum(as.numeric(unlist(str_extract_all(gsub("\\(\\d\\)","",score), "[0-9]+")))),minutes),by=c(colnames(Tennis_table))])[(anydate(i_date)>anydate(tourney_date))][difftime(anydate(i_date),anydate(tourney_date))<=i_duree]
historique
historique[,.(scorenum,minutes)][,lapply(.SD, function(x) sum(na.omit(x[is.finite(x)])))]

f_fatigue(Tennis_table,i_name,i_date,100)[,scorenum]

start_time <- Sys.time()
Table_Fatigue=Tennis_table[,.(fatigue=(f_fatigue(Tennis_table,winner_name,tourney_date,7)[,scorenum]-f_fatigue(Tennis_table,loser_name,tourney_date,7)[,scorenum])),by=c(colnames(Tennis_table))]
end_time <- Sys.time()
time=end_time - start_time

nrow(Table_Fatigue[fatigue>0])
nrow(Table_Fatigue[fatigue<0])
