...
>>>>>>> e4782195f5f8ceab492ea1bc111f832f009f6ee9
...
for(i in AnneeDeb:AnneeFin){
  ...
  # res_test<-test$coin
  xgboost_mod<- xgboost(data=Xtrain,label = Ytrain, max.depth=25, eta=0.025, nthread=2,nrounds=15,objective="binary:logistic")
  modeles[[i-AnneeDeb+1]]<-xgboost_mod
  y_pred<-predict(xgboost_mod,Xtest)
  y_pred2<-lapply(y_pred,FUN = func)
  Resultats[i-AnneeDeb+1]<-sum(as.numeric(y_pred2==Ytest))/length(y_pred2)
  
   xgboost_mod1<- xgboost(data=Xtrain,label = Ytrain, max.depth=25, eta=0.05, nthread=2, nrounds=40,objective="binary:logistic")
  modeles1[[i-AnneeDeb+1]]<-xgboost_mod
  y_pred<-predict(xgboost_mod1,Xtest)
  y_pred2<-lapply(y_pred,FUN = func)
  Resultats1[i-AnneeDeb+1]<-sum(as.numeric(y_pred2==Ytest))/length(y_pred2)

   xgboost_mod2<- xgboost(data=Xtrain,label = Ytrain, max.depth=25, eta=0.1, nthread=2,nrounds=32,objective="binary:logistic")
  modeles2[[i-AnneeDeb+1]]<-xgboost_mod
  y_pred<-predict(xgboost_mod2,Xtest)
  y_pred2<-lapply(y_pred,FUN = func)
  Resultats2[i-AnneeDeb+1]<-sum(as.numeric(y_pred2==Ytest))/length(y_pred2)
  
  
   xgboost_mod3<- xgboost(data=Xtrain,label = Ytrain, max.depth=25, eta=0.2, nthread=2,nrounds=25,objective="binary:logistic")
  modeles3[[i-AnneeDeb+1]]<-xgboost_mod
  y_pred<-predict(xgboost_mod3,Xtest)
  y_pred2<-lapply(y_pred,FUN = func)
  Resultats3[i-AnneeDeb+1]<-sum(as.numeric(y_pred2==Ytest))/length(y_pred2)
