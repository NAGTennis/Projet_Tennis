...
RES["Ridge2"] <- predict(cv_mod3,Xtest,type='class',s=c(cv_mod3$lambda.min)) #<------ Pourquoi Eric r?cup?re le lambda.min de la ridge (dans son exemple ? lui)?
...
RES["ElNet"]  <- predict(cv_mod4,Xtest,type='class',s=c(cv_mod4$lambda.min))
...
save(mod1,file = "Reglog.RData")
save(cv_mod2,file = "Ridge.RData")
save(cv_mod4,file = "Lasso.RData")
save(cv_mod3,file = "ElNet.RData")
save(xgboost_mod,file = "XGB.RData")
